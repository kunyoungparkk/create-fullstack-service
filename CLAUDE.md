...
페이지별 기능을 쌓기 전에 토대를 먼저 깐다.

1. 화면이 아니라 도메인(예: `todo`, `user`)으로 경계를 나눈다.
2. 도메인마다 데이터 계층을 먼저 세운다 — 클라이언트는 `api/<도메인>`·
   `hooks/<도메인>`(`state-management` 스킬), 서버는 도메인당 한 모듈(`nestjs.md`).
3. 그 위에 페이지 기능을 올린다. 페이지는 토대를 소비할 뿐 데이터 접근을 직접 하지 않는다.
